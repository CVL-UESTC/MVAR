...
class VectorQuantizer2(nn.Module):
    ...
    # ===================== idxBl_to_mvar_input: only used in MVAR training, for getting teacher-forcing input =====================
    def idxBl_to_mvar_input(self, gt_ms_idx_Bl: List[torch.Tensor], SN=None) -> torch.Tensor:
        next_scales = []
    ...
    # ===================== get_next_autoregressive_input: only used in MVAR inference, for getting next step's input =====================
    def get_next_autoregressive_input(
        self, si: int, SN: int, f_hat: torch.Tensor, h_BChw: torch.Tensor
    ) -> Tuple[Optional[torch.Tensor], torch.Tensor]:  # only used in MVAR inference
