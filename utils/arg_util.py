...
class Args(Tap):
    ...
    # MVAR
    tfast: int = (
        0  # torch.compile MVAR; =0: not compile; 1: compile with 'reduce-overhead'; 2: compile with 'max-autotune'
    )
    ...
    tclip: float = 2.0  # <=0 for not using grad clip
    ls: float = 0.0  # label smooth

    bs: int = 64  # global batch size, original is 768
    batch_size: int = (
    ...
    local_out_dir_path: str = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "MVAR-log"
...
def init_dist_and_get_args():
    ...
    ### update args.local_out_dir_path ###
    args.local_out_dir_path = os.path.join(args.local_out_dir_path, args.exp_name)
    ...
    args.last_ckpt_path = os.path.join(args.local_out_dir_path, f"mvar-ckpt-last.pth")
    _reg_valid_name = re.compile(r"[^\w\-+,.]")
    tb_name = _reg_valid_name.sub(
        "_",
        f"tb-MVARd{args.depth}"
        f"_pn{args.pn}"
        f"_b{args.bs}ep{args.ep}{args.opt[:4]}lr{args.tblr:g}wd{args.twd:g}",
    )
