...
from utils import arg_util, dist
...
class MetricLogger(object):
    ...
    def cache_log_every(self, iterable, print_freq, header=None):
        i = 0
        if not header:
            header = ""
        start_time = time.time()
...
def create_npz_from_sample_folder(sample_folder: str, total_samples: int = 50000):
    """
    ...
    assert (
        len(pngs) == total_samples
    ), f"{len(pngs)} png files found in {sample_folder}, but expected {total_samples}"
