...
class MVARTrainer(object):
    ...
    @torch.no_grad()
    def eval_ep(self, ld_val: DataLoader, use_cached: bool = True):
        tot = 0
        ...
        for data in ld_val:
            if not use_cached:
                inp_B3HW, label_B = data[0], data[1]
                ...
                gt_BL, x_BLCv_wo_first_l, label_B = data[0], data[1], data[2]
    ...
    def train_step(
        ...
        *,
        ...
        assert (
            prog_ed > 0
        ), f"prog_ed must be greater than 0, but got prog_ed = {prog_ed}"
        ...
        *,
        ...
        for it, data in me.log_every(
            ...
            if cur_prog < ratio_k:
                ...
                prog_ed = (
                    self.refine_step + cur_prog % ratio_k
                    if ratio_k != 1
                    else self.refine_step + (cur_prog - 1)
                )
        ...
        for k in ("mvar_wo_ddp", "mvar_opt"):
            ...
            if m is not None:
                ...
                if ret is not None:
                    ...
                    print(
                        f"[MVARTrainer.load_state_dict] {k} unexpected:  {unexpected}"
                    )
